    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-0">
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="flex flex-1 items-center justify-center sm:justify-start gap-2">
          <div class="flex flex-shrink-0 items-center cursor-pointer" @click="handleRoute('/')" aria-hidden="true">
            LOGO
          </div>
          <span v-if="userStore.isAuthenticated" class="flex flex-1 items-center justify-center sm:justify-start gap-14">
            <div class="hidden sm:ml-6 sm:block">
              <div class="flex space-x-4">
                <span @click="handleRoute('/saved')" aria-hidden="true" :class="[
                  isActive('/saved')
            ...
            <form class="w-2/4">
              <label for="default-search"
                class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Qidiring</label>
              <div class="relative">
            ...
            <transition v-if="showMenu" enter-active-class="transition ease-out duration-100"
              ...
              <div v-if="userStore.isAuthenticated"
                ...
                <button @click="handleLogout"
                  class="text-red-500 block px-4 py-2 text-sm hover:bg-gray-700 w-full text-left" id="logout">
                  Logout
                </button>
              </div>
              <div v-else
                class="absolute right-0 z-20 mt-2 w-54 origin-top-right rounded-md bg-gray-800 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <router-link class="text-white block px-4 py-2 text-sm" :class="{ 'text-gray-900': isActive('/sign-in') }"
                  to="/sign-in" id="menu-item-0">Sign In</router-link>
                <router-link class="text-white block px-4 py-2 text-sm" :class="{ 'text-gray-900': isActive('/sign-up') }"
                  to="/sign-up" id="menu-item-1">Register</router-link>
              </div>
            </transition>
          </div>
        </div>
