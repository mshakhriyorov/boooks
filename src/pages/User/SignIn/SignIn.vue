    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        <h2 class="mt-6 text-center text-3xl font-extrabold">
          {{ $t("SignIn.signInYourAccount") }}
        </h2>
        <p class="mt-2 text-center text-sm text-gray-600">
          {{ $t("SignIn.or") }}
          <router-link class="font-medium text-indigo-600 hover:text-indigo-500" to="/sign-up">
            {{ $t("SignIn.create") }}
          </router-link>
        </p>
      </div>
      <form class="mt-8 space-y-6" action="#" method="POST" @submit.prevent="handleSubmit">
        <div class="rounded-md shadow-sm -space-y-px">
          <div>
            <label for="email-address" class="sr-only">{{ $t("SignIn.email") }}</label>
